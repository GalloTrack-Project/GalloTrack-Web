...
export default function DeceasedModal({
  ...
  return (
    <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[999] flex items-center justify-center p-4 animate-fadeIn">
      <div className="bg-white dark:bg-card rounded-3xl shadow-2xl border border-rose-200 max-w-md w-full p-6 space-y-5 relative">
        <button 
        ...
        <div className="flex items-center space-x-3 text-rose-700 border-b pb-3 border-rose-100">
          <div className="w-10 h-10 bg-rose-100 rounded-2xl flex items-center justify-center"><Skull className="w-5 h-5 text-rose-600" /></div>
          <div>
            <h3 className="text-base font-black text-slate-900 dark:text-card-foreground tracking-tight">Record Mortality</h3>
            <p className="text-[11px] text-slate-500 dark:text-muted-foreground font-semibold">Transition node to Deceased status — cause of death required</p>
          </div>
        </div>
        ...
        <div className="space-y-2">
          <label className="block text-[11px] font-extrabold text-slate-700 dark:text-card-foreground uppercase tracking-wider">Cause of Death</label>
          <select 
            value={deathReasonInput} 
            onChange={(e) => setDeathReasonInput(e.target.value)} 
            className="w-full p-3 border border-slate-200 dark:border-border rounded-xl text-xs bg-slate-50 dark:bg-muted font-extrabold text-slate-800 dark:text-card-foreground outline-none focus:border-rose-500 cursor-pointer"
          >
